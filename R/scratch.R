...
fits_long <- function(fits_attr, salgrd) {
  fits_attr |>
    pivot_longer(cols = starts_with('X'), names_to = 'sal_lev', values_to = 'chla_pred') |>
...
p1 <- ggplot(toplo_act, aes(x = year, y = chla_pred, group = sal, color = sal)) +
  geom_line(linewidth = 0.7, alpha = 0.7) +
  geom_line(
    data = normsann, aes(x = yr, y = btnorm),
    inherit.aes = FALSE, color = 'black', linewidth = 1.5
  ) +
  scale_color_viridis_c(name = 'Salinity (ppt)', option = 'mako', direction = -1) +
  scale_y_continuous(limits = ylim_sal) +
  labs(
    y = 'Chl-a (µg/L)',
    subtitle = 'A. Predictions at actual loading\nColored lines = 10 salinity levels; black = mean across range'
  ) +
  thm_top
...
p2 <- ggplot(toplo_md, aes(x = year, y = chla_pred, group = sal, color = sal)) +
  geom_line(linewidth = 0.7, alpha = 0.7) +
  geom_line(
    data = normsann, aes(x = yr, y = btnormmd),
    inherit.aes = FALSE, color = 'black', linewidth = 1.5
  ) +
  scale_color_viridis_c(name = 'Salinity (ppt)', option = 'mako', direction = -1) +
  scale_y_continuous(limits = ylim_sal) +
  labs(
    y = 'Chl-a (µg/L)',
    subtitle = 'B. Predictions at mean loading\nColored lines = 10 salinity levels; black = mean across range'
  ) +
  thm_top
...
p3 <- ggplot(normsann, aes(x = yr)) +
  ...
  geom_line(aes(y = btnormmd, color = 'Normalized, Mean Load', linetype = 'Normalized, Mean Load')) +
  scale_linetype_manual(values = c('Normalized' = 'solid', 'Normalized, Mean Load' = 'dashed')) +
  scale_color_manual(
    values = c('Normalized' = 'tomato1', 'Normalized, Mean Load' = 'dodgerblue3')
  ) +
  scale_fill_manual(values = c('Predicted' = 'black')) +
  labs(
    x = NULL, y = 'Annual mean chl-a (µg/L)',
    subtitle = 'C. Resulting normalized time series',
